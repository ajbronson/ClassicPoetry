...
class FileController {
    ...
    func volumes() -> [Volume] {
        return try! dbQueue.inDatabase({ (db: Database) -> [Volume] in
            ...
            for row in try Row.fetchAll(db, "select * from canon order by name") {
    ...
    func book(volumeID: Int) -> [Book] {
        return try! dbQueue.inDatabase({ (db: Database) -> [Book] in
            ...
            if volumeID == 0 {
                for row in try Row.fetchAll(db, "select p.*, c.name from poems p left join canon c on c.id = p.canon_id order by c.name") {
